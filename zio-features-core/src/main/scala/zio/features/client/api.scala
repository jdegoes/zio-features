...
object api extends ClientApis with CommonApis {

  /**
   * {{{
   * object MyApp extends ZIOAppDefault {
   *   val run = myApp.provide(zio.features.client.api.layer)
   * }
   * }}}
   */
  val layer: ZLayer[ZioFeaturesServerConfig, Nothing, Features] =
    ZLayer.makeSome[ZioFeaturesServerConfig, Features](
      FeaturesImpl.layer,
      FeatureDecisionMatrixRepoClient.layer,
      FeatureDescriptorRepoClient.layer
    )
